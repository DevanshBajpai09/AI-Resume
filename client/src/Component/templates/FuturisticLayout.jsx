...
const FuturisticTemplate = ({ data, accentColor = "#3B82F6" }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white rounded-xl overflow-hidden shadow-xl">
      ...
      {/* Decorative */}
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-gradient-to-tr from-indigo-500/5 to-cyan-500/5 rounded-full translate-y-40 -translate-x-40"></div>
                        ...
                        <div className="mt-2 space-y-1 text-xs text-gray-300">
                          {exp.description.split("\n").map((line, i) => (
                            <div key={i} className="flex gap-2">
                              <ChevronRight className="w-3 h-3 mt-1" style={{ color: accentColor }} />
                              <span>{line}</span>
                            </div>
                          ))}
                        </div>
